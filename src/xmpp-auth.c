...
static int _handle_stream_error(xmpp_conn_t *conn, xmpp_stanza_t *stanza, void *userdata)
{
    ...
    if (conn->stream_error) {
        ...
        do {
            ...
            if (child) {
                ns = xmpp_stanza_get_ns_ptr(child);
            }
...
static int _handle_features(xmpp_conn_t *conn, xmpp_stanza_t *stanza, void *userdata)
{
    ...
    if (!conn->secured) {
        if (!conn->tls_disabled) {
            ...
            if (child && (strcmp(xmpp_stanza_get_ns_ptr(child), XMPP_NS_TLS) == 0))
                conn->tls_support = 1;
        } else {
            // 服务器不支持tls
            conn->tls_support = 0;
    ...
    if (child && (strcmp(xmpp_stanza_get_ns_ptr(child), XMPP_NS_SASL) == 0)) {
    ...
    if (child && (strcmp(xmpp_stanza_get_ns_ptr(child), XMPP_NS_COMPRESSION) == 0)) {
        for (mech = xmpp_stanza_get_children(child); mech;
             mech = xmpp_stanza_get_next(mech)) {
...
static int _handle_features_sasl(xmpp_conn_t *conn, xmpp_stanza_t *stanza, void *userdata)
{
    ...
    if (bind && strcmp(xmpp_stanza_get_ns_ptr(bind), XMPP_NS_BIND) == 0) {
        conn->bind_required = 1;
    }

    // 检查服务器是否需要session
    session = xmpp_stanza_get_child_by_name(stanza, "session");
    if (session && strcmp(xmpp_stanza_get_ns_ptr(session), XMPP_NS_SESSION) == 0) {
